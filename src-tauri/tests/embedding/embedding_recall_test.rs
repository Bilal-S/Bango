...
fn recall_max_pool_picks_best_chunk_per_article() {
    ...
    let rows = embedding_repo::list_for_recall(&conn, 4, None, &["included".to_string()]).unwrap();
...
fn recall_max_pool_seed_handles_negative_similarity() {
    ...
    let rows = embedding_repo::list_for_recall(&conn, 2, None, &["included".to_string()]).unwrap();
