...
/// The frontend uses this to decide whether to render the embed iframe
/// (avoiding a guaranteed 429) or to surface a fallback UI.
...
///
/// Uses `Range: bytes=0-0` rather than HEAD because Google's CDN responds
/// more reliably to GET, and we only need the status code.
///
/// Issued from the Rust side via `reqwest` which is NOT subject to Tauri's
/// HTTP capability system, so no capability grant is required. This keeps
/// the probe invisible to Google's iframe anti-bot logic while still
/// exercising the same rate-limit counters.
#[tauri::command]
