...
pub use query::{
    ...
    get_articles_by_status, get_articles_by_statuses, get_articles_for_export,
    get_duplicate_articles, get_working_articles, query_articles, ArticleQuery,
};
...
pub(super) fn row_to_article(row: &rusqlite::Row<'_>) -> rusqlite::Result<Article> {
    ...
    Ok(Article {
        ...
        data_length: row.get::<_, Option<i64>>("data_length")?.map(|v| v as usize),
        token_estimate: row.get::<_, Option<i64>>("token_estimate")?.map(|v| v as usize),
        actual_tokens: row.get::<_, Option<i64>>("actual_tokens")?.map(|v| v as usize),
