...
pub fn format_paper_label(authors_str: &str, year: Option<i32>) -> String {
    ...
    let first = parsed[0].display_name.split(',').next().unwrap_or(&parsed[0].display_name).trim();
    if parsed.len() == 1 {
        format!("{}{}", first, year_suffix)
    } else {
        format!("{} et al.{}", first, year_suffix)
    }
}
...
///
/// This closes a gap where references imported without auto-matching (or whose
/// matches were missed due to import ordering) would never appear in the
/// citation network.  Runs idempotently inside the `biblio_normalize`
/// transaction before [`build_citation_edges`].
///
