...
/// A project-portable `app_settings` row (see `PROJECT_PORTABLE_SETTINGS` allowlist). Machine-local state stays put.
...
pub struct ProjectBackup {
    ...
    /// Translation originals archive. Survives backup/restore.
    #[serde(default)]
    ...
    /// Project-portable `app_settings` rows. `#[serde(default)]` for old-backup compat.
    #[serde(default)]
...
pub fn export_project(conn: &Connection) -> Result<String, AppError> {
    ...
    //
    /* The 9 biblio_* tables are NOT exported — dynamically generated by
    biblio_normalize from the source tables. Exporting bloats the backup and can
    trigger UNIQUE constraint violations. After import, mark_biblio_needs_refresh
    triggers auto-regeneration via biblio_normalize. */
    ...
    /* Filter out genuine orphan audit entries (article_id references a non-existent
    article). Runtime deletes cascade via ON DELETE CASCADE; this is defense-in-depth
    for orphans created while foreign_keys were OFF. System-level rows preserved in
    both shapes: article_id IS NULL (current log_error path) and article_id = ''
    (historical, normalized to NULL by v006 migration). Dropping either deletes
    ...
    /* Translation originals archive. Serialized so backup/restore preserves
    original-language text + chunks for traceability and reprocessing. */
    ...
    /* Project-portable app_settings (screening rules, summary mode, auto-translate,
    screening-mode params). Only allowlisted keys travel; machine-local state excluded. */
...
/// ID-remap maps. Filled by UNIQUE-constrained table importers; read by dependent junction importers.
#[derive(Default)]
struct ImportMaps {
...
/// Single-column UNIQUE lookup for `biblio_authors` / `biblio_institutions`. On INSERT OR IGNORE skip, find existing row by its unique column.
///
/// # Security
///
/// `table` + `unique_col` MUST be compile-time-constant string literals (never user input).
...
pub fn import_project(conn: &Connection, json_str: &str) -> Result<(), AppError> {
    ...
    /* Post-import: resolve journal links. Backup files omit journal_index_id
    (derived from ISSN/eISSN/name), so rematch against the restoring machine's journal_index. */
...
/// Import audit entries. Empty-string `articleId` → NULL (v006 FK constraint).
...
/// Import project-portable app_settings. `project_name`: absent → clear target (dashboard reverts to fallback).
...
/// Remap reference paper IDs when INSERT OR IGNORE skips a duplicate (by DOI, then title+authors+year).
