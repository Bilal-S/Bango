...
///
/// Tags and labels must be concise descriptors, not full justifications.
/// The screening system prompt instructs the LLM to keep tags ≤ 35 chars;
/// this is the backend enforcement that strips prefixes and truncates at
/// word boundaries so a too-long name never silently loses context
/// (e.g. `"inclusion: this is a very long cr"` → `"this-is-a-very-long"`).
const MAX_NEW_TAG_LABEL_LEN: usize = 35;
...
const TAG_LABEL_PREFIXES: &[&str] = &["inclusion:", "exclusion:", "inclusion -", "exclusion -"];
...
/// Truncate at a word boundary (hyphen) so the result is at most `max_len` chars.
///
/// Pure helper (`#[must_use]`) used by `create_or_match_tag`/`create_or_match_label`
/// to avoid cutting names mid-word.
#[must_use]
