...
|---|---|
| `src-tauri/tests/search_strategy_test.rs::build_prompt_includes_aims_text` | The user prompt embeds each research aim's text. |
| `src-tauri/tests/search_strategy_test.rs::build_prompt_includes_all_eight_databases` | The system prompt names all 8 Boolean databases (PubMed, Scopus, Web of Science, Cochrane, EBSCOhost, JSTOR, ScienceDirect, arXiv). |
| `src-tauri/tests/search_strategy_test.rs::build_prompt_includes_arxiv_andnot` | The system prompt carries the arXiv-specific `ANDNOT` operator guidance. |
| `src-tauri/tests/search_strategy_test.rs::build_prompt_includes_semantic_scholar_advisory` | The system prompt emits the Semantic Scholar non-Boolean advisory. |
...
| `src-tauri/tests/search_strategy_test.rs::build_prompt_includes_harmonization_guidance` | The user prompt warns that negating exclusions are redundant and must not be encoded as self-canceling NOT clauses. |
