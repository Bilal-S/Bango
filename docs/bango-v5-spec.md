...
* **One call path**: all generation still flows through `LlmOrchestrator`; a `LocalConfigProvider` resolves the live loopback config (ephemeral port, per-start random API key) at send time. Local requests run serialized (concurrency 1, no pacing) under a single 1800 s wall-clock override of every per-type cloud timeout. Features fetch config through the backend-aware resolver, so a stored cloud context window never sizes a local prompt; context is RAM-aware (16k below 24 GB, 32k at 24 GB+, 64k at 48 GB+; selectable 8k/16k/32k/64k, machine-local).
* **Structured output**: `send_json` sends `response_format: json_object` on the local path only (cloud transports never receive the field). Structured request types force thinking off via `chat_template_kwargs.enable_thinking = false`; a "Deeper reasoning for prose tasks" preference (default off) applies only to prose requests. Because llama.cpp's `json_object` grammar cannot emit a bare top-level array, local JSON consumers request object wrappers (e.g. Citation Finder's `results` / `claims` keys) and their parsers recover any object shape that carries the payload array.
* **Lifecycle**: single-flight lazy start with `/health` gating, native `--sleep-idle-seconds` model unload in place, one bounded crash restart, stop on backend switch (an in-flight generation finishes first), component removal, and app exit. Install state is derived (Ready / RepairRequired / NotInstalled) from pinned sizes plus install manifests, never persisted; Verify/Repair/Remove are idempotent, with a locked-Windows trash rename for removals.
