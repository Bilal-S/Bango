...
* **Project Backup**: Single `.bango.json` containing aims, criteria, articles, tags, labels, audit logs, reference papers/links, and a curated portable subset of `app_settings` (§8.1). Secrets are never exported. On import, a locally defined LLM connection (provider + encrypted API key, per the `has_config` contract) is machine-local state and is kept untouched; the backup's provider/endpoint/model triple restores only when no usable local connection exists (fresh machine).
