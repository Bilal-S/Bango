...
* **Conflict Resolver** (applies only when no Custom Screening Instructions are present):
    ...
    5. Failed-inclusion guard: a required inclusion criterion the LLM marked as NOT met (an inclusion key in the matched-exclusion array) that strictly outranks every satisfied inclusion forces exclusion even when the LLM's `decision` field says include; equal priority keeps the tie-favors-inclusion rule.
...
* **Readiness Check**: Requires ≥ 1 aim, ≥ 1 inclusion, ≥ 1 exclusion criterion, valid LLM config, and worst-case per-article token estimation. Warns if any estimated footprint exceeds 80% of `contextWindowTokens` (minimum: 16,000). The footprint is mode-aware (see §4.3.1). `contextWindowTokens` is user-editable per model; switching the provider in Settings seeds a provider-appropriate default (OpenAI 128K, Anthropic 200K, Google 1M, Mistral/Z.AI 128K, local providers 50K).
...
* **`embedding_backend`**: Which backend generates embeddings: `configured_provider` (default, follows the configured AI provider) or `bango_local` (on-device inference via the downloaded local model + ONNX Runtime). Portable: travels with project backups; readiness for a restored selection is re-evaluated per machine (a restored `bango_local` without installed components hits the contextual install prompt). Unrecognized values fall back to the default.
* **`llm_backend`**: Which backend serves generation calls: `configured_provider` (default, follows the configured AI provider) or `bango_ai` (on-device inference via the downloaded llama.cpp runtime + Qwen3.5-2B model, §8.9). Portable: travels with project backups; readiness for a restored selection is re-evaluated per machine. The stored cloud `llm_config` row is never rewritten by Bango AI, and unrecognized values fall back to the default. The engine settings `bango_ai_context` / `bango_ai_threads` / `bango_ai_reasoning` are machine-local.
...
**Portability contract**: Machine-local settings (`storage_root`, `*_needs_refresh` flags, `wiki_dir_hash`, `openalex_api_key`, the `embedding_status`/`embedding_model`/`embedding_dimensions` triple, `embedding_model_override`, the `bango_ai_*` engine settings) are deliberately excluded from backups; `embedding_backend` and `llm_backend` are portable. Secrets are never exported.
...
- **Manual mode** (always available): free-text Boolean query with filters (year range, work type, language, OA toggle, show-retracted toggle). `has_abstract:true` always appended; `is_retracted:false` default. **DOI-direct**: when the query text is a DOI (bare `10.x/y`, `doi:` scheme, or doi.org/dx.doi.org URL form), the search fetches that exact work via `filter=doi:https://doi.org/{canonical}` instead of `search=`; the DOI-direct URL bypasses `has_abstract:true`, all optional filters (year/type/language/OA), and retraction filtering so the exact work is never silently hidden (retraction stays visible via the result badge; a DOI with no OpenAlex record surfaces the empty state, not an error). A cosmetic hint under the search box mirrors the backend detection.
...
### 8.9 Bango AI (On-Device Generation)

Bango AI is a second generation backend that runs inference entirely on the user's machine: a Bango-managed `llama-server` (llama.cpp `b10964`, pinned and SHA-256 verified) serving the Apache-2.0 `Qwen3.5-2B-UD-Q4_K_XL` GGUF (~1.34 GB, an unsloth dynamic quant of `Qwen/Qwen3.5-2B`). Neither artifact ships in the installer; both download on explicit consent after Bango AI is selected, in runtime-first order (the 11-18 MB executable smoke precedes the model download), and the backend activates only after an engine self-test succeeds. Cancellation or failure never activates Bango AI: the install transaction leaves the previously active backend untouched, and the Settings UI returns the selection to Configured Provider when a download is cancelled.

* **Backend selection**: machine setting `llm_backend` (`configured_provider` default | `bango_ai`) travels with project backups while readiness is re-evaluated per machine. The stored cloud `llm_config` row is never rewritten; switching back restores the provider untouched. There is never a silent fallback from local to cloud: unavailable Bango AI blocks generation with an actionable error (unsupported targets are refused at install and blocked at runtime).
* **One call path**: all generation still flows through `LlmOrchestrator`; a `LocalConfigProvider` resolves the live loopback config (ephemeral port, per-start random API key) at send time. Local requests run serialized (concurrency 1, no pacing) under a single 3600 s (60 min) wall-clock override of every per-type cloud timeout, because many CPUs are slow; per-call output caps bound the generation itself. Features fetch config through the backend-aware resolver, so a stored cloud context window never sizes a local prompt; context is RAM-aware (16k below 16 GB, 32k at 16 GB+, 64k at 32 GB+; selectable 8k/16k/32k/64k, machine-local).
* **Structured output**: `send_json` sends `response_format: json_object` on the local path only (cloud transports never receive the field). Structured request types force thinking off via `chat_template_kwargs.enable_thinking = false`; a "Deeper reasoning for prose tasks" preference (default off) applies only to prose requests. Because llama.cpp's `json_object` grammar cannot emit a bare top-level array, local JSON consumers request object wrappers (e.g. Citation Finder's `results` / `claims` keys) and their parsers recover any object shape that carries the payload array. Local calls also carry a per-request-type `max_tokens` output cap (wiki batches 8192, corpus reports/CitationFinder/chat 4096, summaries 3072, classification 2048; prose calls with reasoning enabled get 2x headroom for thinking tokens) so a runaway/no-EOS generation cannot burn the full 3600 s timeout; a capped response's `finish_reason` drives the existing truncation handling, and a repeated-slug repetition guard plus bounded continuations keep wiki batches bounded.
* **Bango AI wiki behavior**: the per-article full-text AI-summary pre-pass and the per-framework LLM polish are skipped on the local backend (deterministic author/synthesis/concept/method/source pre-seed plus abstract fallback keep the wiki correct, and the ingest report warns when full-text articles fell back to abstracts). Wiki batches are sized for the local output cap (two sources per batch). Every wiki run alternates progress messages between a batch counter with the current-batch age (`Generating (batch X/Y, M:SS) via LLM...`, gaining `slow - limit 60:00` past the threshold) and a time estimate, and Stop aborts an in-flight generation within the tick instead of waiting for the batch to finish; this liveness/ETA/cancel behavior applies to configured-provider runs too (with the provider's 10:00 limit). A cancelled ingest leaves the wiki marked stale so the next Update retries.
* **Lifecycle**: single-flight lazy start with `/health` gating, native `--sleep-idle-seconds` model unload in place, one bounded crash restart, stop on backend switch (a 5 s fast drain, then a detached teardown capped at 30 min - separate from the 60 min request budget - after which the server stops anyway and the in-flight request fails), component removal, and app exit. Install state is derived (Ready / RepairRequired / NotInstalled) from pinned sizes plus install manifests, never persisted; Verify/Repair/Remove are idempotent, with a locked-Windows trash rename for removals.
* **Settings UX**: a backend selection header (radio-style, mirroring the Embeddings card) with plain-language setup copy, human-readable failure categories, Component Details, Test Bango AI timings (model load, first response, tokens/s, effective context), machine-local engine settings (`bango_ai_context`, `bango_ai_threads`, `bango_ai_reasoning` - never portable), and install progress with a single overall meter plus per-component (AI engine / AI model) text byte counts, and cancel/resume, with an explicit note that local inference is generally slower than cloud providers. Selecting Bango AI replaces the provider configuration with the Bango AI panel (the provider card returns on Configured Provider), and a consent install activates in place: the selection is persisted and the live orchestrator switches to the local engine only after the self-test passes. Cancelling the consent dialog or an in-flight download clears the progress UI immediately, returns the selection to Configured Provider with no error, and leaves the partial download resumable.
...
* **Master-Detail Article Viewer**: 3-pane layout with navigation sidebars, filterable/sortable tables, and sliding article detail panels (metadata, AI screening cards, audit timelines).
  ...
  The Author field offers debounced autocomplete: a dropdown of matching author names appears after the user stops typing for 500ms and the fragment is at least 2 characters (case-insensitive substring match).
  The toolbar search accepts field prefixes that route the text to the (possibly collapsed) panel filter: `a:` author (any author or co-author), `d:`/`doi:` DOI, `j:` journal, and `y:`/`year:` a single year or range (`y:2018-2021`; invalid values fall back to the plain search).
  A prefix search mirrors its value into the panel field (same as the `?author=` deep-link), and clearing the toolbar search reverts it; plain text searches title, abstract, and notes.
...
* **Shared Export Dialog**: Opened from the article-list toolbar `Export` button and the PRISMA view's `Export Data` button.
  Options appear in a fixed order - RIS, BibTeX, Zotero, Project Backup, Wiki Website, Obsidian - all rendered with the same secondary button style (BibTeX appears on the Included default surface only; the tab-aware article-list surface keeps RIS + Zotero).
* **RIS Export**: Exports the Included list, mapping AI tags to `KW`, notes to `NO`, labels to `C8` JSON groupings.
* **BibTeX Export**: Exports the Included list as a `.bib` file for LaTeX manuscripts (Included-list only in v1; tab/ids variants are a non-goal) with full field parity to the RIS export: abstract in `abstract`, language, issn, imported notes in `annote`, AI reasoning in `comment`, user notes in `note` (note: `note` renders in standard bibliography styles), `keywords` as the RIS `KW` union (plain keywords plus `Bango:`-prefixed tags and labels), and the RIS `C8` criteria payload mirrored in a custom `bango-criteria` field with raw balanced-JSON braces. Citation keys are `{first-author-surname}{year}{first-significant-title-word}` (ASCII-folded, lowercased; `anon`/`nd` fallbacks; letter-suffixed `b`, `c`, ... on collision within one file). A lone start page (elocation ids like `e12345`) passes through verbatim; entry types reverse-map from the RIS-style type (unknown defaults to `@article`); user-text braces are escaped, percent/backslash left as-is; UTF-8 without BOM.
...
* **Project Backup**: Single `.bango.json` containing aims, criteria, articles, tags, labels, audit logs, reference papers/links, and a curated portable subset of `app_settings` (§8.1), including the embedding backend choice. Secrets are never exported. On import, a locally defined LLM connection (provider + encrypted API key, per the `has_config` contract) is machine-local state and is kept untouched; the backup's provider/endpoint/model triple restores only when no usable local connection exists (fresh machine).
...
* **Obsidian Vault Export**: Packages the current wiki pages as a Markdown vault with all UUIDs pre-processed, ready to open as an Obsidian vault, in a `.zip` via the native save dialog. Synthesis pages are renamed to `{first-author-surname}{year}{title-word}` slugs (the shared BibTeX citation-key convention: ASCII-folded, `anon`/`nd` fallbacks, letter-suffix dedup across the export, plus a cross-type filename collision guard); `[[uuid]]` wikilinks, `[^art-<uuid>]` footnote refs/definitions, bare `/raw/<uuid>.md` paths, and the `id`/`slug`/`source_articles`/`links` frontmatter are rewritten to the slugs (unmapped UUIDs - deleted articles - are dropped, alias text kept); internal `source_file`/`source_hash` provenance plus `raw/`, `templates/`, `log.md`, and `wiki/index.md` are excluded; a generated `Home.md` index and a minimal `.obsidian/` config (`app.json` + `graph.json` color groups mirroring the app page-type colors) are included. Pre-processing runs entirely in the Rust backend (`wiki_export_obsidian`, single-step: stage to `wiki-root/obsidian-export/` then zip). Reachable from both the Wiki toolbar `Actions` menu ("Export to Obsidian", disabled until the wiki is initialized) and the shared export dialog. DB article full text/PDFs are never included (copyright-safe, consistent with the static-site export).
* **Start-New-Project workflow**: Because Bango is single-project, starting a new review is a guided back-up → delete → begin-fresh flow (Settings → Project Management or Dashboard header link). The Settings "Start New Project" button preserves the machine-local LLM connection (provider + encrypted API key + tuning) across the schema rebuild; "Delete All Data" performs the full wipe including the LLM config. The Help Guide documents three valid entry paths (Aims-first, Articles-first, Search-via-OpenAlex) that converge on the screening/review pipeline.
