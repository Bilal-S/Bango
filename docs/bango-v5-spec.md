...
* **`llm_backend`**: Which backend serves generation calls: `configured_provider` (default, follows the configured AI provider) or `bango_ai` (on-device inference via the downloaded llama.cpp runtime + Ornith model, §8.9). Portable: travels with project backups; readiness for a restored selection is re-evaluated per machine. The stored cloud `llm_config` row is never rewritten by Bango AI, and unrecognized values fall back to the default. The engine settings `bango_ai_context` / `bango_ai_threads` / `bango_ai_reasoning` are machine-local.
...
Bango AI is a second generation backend that runs inference entirely on the user's machine: a Bango-managed `llama-server` (llama.cpp `b10964`, pinned and SHA-256 verified) serving the MIT-licensed `Ornith-1.5-9B-Q4_K_M` GGUF (~5.78 GB). Neither artifact ships in the installer; both download on explicit consent after Bango AI is selected, in runtime-first order (the 11-18 MB executable smoke precedes the model download), and the backend activates only after an engine self-test succeeds. Cancellation or failure leaves the previously active backend untouched.
