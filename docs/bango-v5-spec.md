...
**Portability contract**: Machine-local settings (`storage_root`, `*_needs_refresh` flags, `wiki_dir_hash`, `openalex_api_key`, the `embedding_status`/`embedding_model`/`embedding_dimensions` triple, `embedding_model_override`) are deliberately excluded from backups; `embedding_backend` is portable. Secrets are never exported.
