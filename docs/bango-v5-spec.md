...
Bango AI is a second generation backend that runs inference entirely on the user's machine: a Bango-managed `llama-server` (llama.cpp `b10964`, pinned and SHA-256 verified) serving the Apache-2.0 `Qwen3.5-2B-UD-Q4_K_XL` GGUF (~1.34 GB, an unsloth dynamic quant of `Qwen/Qwen3.5-2B`). Neither artifact ships in the installer; both download on explicit consent after Bango AI is selected, in runtime-first order (the 11-18 MB executable smoke precedes the model download), and the backend activates only after an engine self-test succeeds. Cancellation or failure leaves the previously active backend untouched.
