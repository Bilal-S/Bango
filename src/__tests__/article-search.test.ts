...
import { makeArticlesStore, makeTagsStore, makeLabelsStore } from './helpers/fixtures';
...
// ── Mock stores ─────────────────────────────────────────────────────
const mockArticlesStore = makeArticlesStore({
...
/* Refactor1 T5.3: the 19 suite sections below were nested inside one giant
 * describe('useArticleSearch') callback (1629 lines). They are now top-level
 * describes sharing the module-scope beforeEach; test names drop the
 * redundant 'useArticleSearch >' prefix. */
...
describe('refreshArticle', () => {
  it('fetches the article, patches the articles list row, and fetches counts', async () => {
    ...
    const { search, selectArticle, refreshArticle, articles, selectedArticle } = useArticleSearch();
    await search();
    await selectArticle('a1');
